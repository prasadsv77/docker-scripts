package com.broadcom.clarity.validator.services.hdp;
...
public class HDPHandlerServiceImpl extends AbstractPrepareMessages implements ApplicationHandlerService
{
  private static Logger logger = LogManager.getLogger( HDPHandlerServiceImpl.class );
  @Autowired
  ...
  {
    Properties hdpPropertiesObject = (Properties) applicationContext.getBean( APP_PROPERTIES );
    try
    {
      if( hdpPropertiesObject != null )
      {
        String _fileJsonContent = HandlerUtilities.getJsonContentFromFile( hdpPropertiesObject.getProperty( Constants.APP_VALIDTION_DEF_JSON_PATH ),
            hdpPropertiesObject.getProperty( Constants.HDP_ENV_VARIABLE_JSON_PROP ), true );

        if( _fileJsonContent != null && HandlerUtilities.isJSONValid( _fileJsonContent ) )
        {
          logger.debug( "JSON File Content " + _fileJsonContent );
          Messages messages = commonValidator.validationForEnvVar( _fileJsonContent, Constants.SupportedApps.hdp.toString() );
          return messages;
        }
      }
    }
    ...
    {
      logger.error( "Error while reading container variable definition file -" + hdpPropertiesObject.getProperty( Constants.APP_VALIDTION_DEF_JSON_PATH )
          + File.separator
          + hdpPropertiesObject.getProperty( Constants.CLRT_ENV_VARIABLE_JSON_PROP ) + " ", e.getMessage() );
  ...
  public void populateMappings(){
    handlerUtilities.populateMappings( Constants.HDP_MAPPING_JSON_PROP  );
  }
}