...
public class CommonValidator
{
        ...
        switch( validation.toString() )
        {
          ...
          case "isValidJasperObject":
            ...
            if (result)
              ...
              return result;
          case "isWithinRange":
            result = isWithinRange( envVariableName, productName, _messages );
            if (result)
              break;
            else
              return result;
 ...
 private boolean isValidJasperObject( String variableName, String productName, Messages _messages )
 {
  ...
  private boolean isWithinRange( String variableName, String productName, Messages _messages )
  {
    String envVariableValue = HandlerUtilities.environmentVariableMap.get( variableName );
    boolean isValidValue = true;
    long envVariableVal =0;
    int minValue =0;
    int maxValue = 100;
    String val ="";
    try {
        envVariableVal = Long.parseLong(envVariableValue);

        switch( variableName )
        {
            case "JS_SCHEDULER_JOB_THREAD_COUNT":
            case "JS_SCHEDULER_JOB_THREAD_PRIORITY":
            case "JS_NON_SCHEDULER_JOB_THREAD_COUNT":
            case "JS_NON_SCHEDULER_JOB_THREAD_PRIORITY":
            {
                minValue =1;
                maxValue=10;
                break;
            }
            case "JS_SCHEDULER_JOB_MISFIRE_THRESHOLD":
            case "JS_NON_SCHEDULER_JOB_MISFIRE_THRESHOLD":
            {
                minValue =60000;
                maxValue=300000;
                break;
            }
            case "JS_SCHEDULER_REPORT_GOVERNOR_MAXPAGES":
            {
                minValue =500;
                maxValue=1500;
                break;
            }
            case "JS_NON_SCHEDULER_REPORT_GOVERNOR_MAXPAGES":
            {
                minValue =100;
                maxValue=700;
                break;
            }
            case "JS_SCHEDULER_PHANTOMJS_TIMEOUT_FOR_HIGHCHARTS":
            case "JS_SCHEDULER_PHANTOMJS_TIMEOUT_FOR_FUSIONCHARTS":
            case "JS_NON_SCHEDULER_PHANTOMJS_TIMEOUT_FOR_HIGHCHARTS":
            case "JS_NON_SCHEDULER_PHANTOMJS_TIMEOUT_FOR_FUSIONCHARTS":
            {
                minValue =300000;
                maxValue=900000;
                break;
        ...
        val = Constants.MsgValues.INVALID_RANGE.toString()+":"+" Given: "+envVariableVal+" :"+" Min: "+minValue+" Max: "+maxValue;
        if (!(envVariableVal >= minValue && envVariableVal <= maxValue)) {
            generateMessages(_messages, variableName, val, "error",
                    productName.toLowerCase() + ".ENV.VARIABLE.".toLowerCase().concat(variableName));
            isValidValue = false;
        }
    }
    catch(NumberFormatException e) {
        generateMessages(_messages, variableName, Constants.MsgValues.INVALID_RANGE.toString(), "error",
                productName.toLowerCase() + ".ENV.VARIABLE.".toLowerCase().concat(variableName));
        isValidValue = false;
    }
